...
let wind = {
  ...
  dir: new Vector3(windPS / 60, windPS / 200, windPS / 60),
};
let addParticle = ({ init = false } = {}) => {
  ...
  particle.mesh.geometry.setAttribute(
    ...
    new BufferAttribute(
      new Float32Array(
        new Array(particle.mesh.geometry.getAttribute('position').count)
...
function step() {
  ...
  particles.forEach((particle, index) => {
    ...
    const turb =
      Math.max(
        0,
        turbulence.noise(particle.mesh.position.x / 20, particle.mesh.position.y / 20, particle.mesh.position.z / 20) -
          0.5,
      ) || 0;
    particle.accelerate(
      new Vector3(
