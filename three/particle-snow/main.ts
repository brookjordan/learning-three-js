...
class Particle {
  ...
  constructor({
    ...
    this.mesh = mesh;
    this.speed = speed ?? new Vector3(0, 0, 0);
    this.color = color;
  }
}
...
function step() {
  ...
  particles.forEach((particle, index) => {
    ...
    if (particle.mesh.position.y < -2 || Math.abs(particle.mesh.position.x) > particleMaxDrift  || Math.abs(particle.mesh.position.z) > particleMaxDrift) {
