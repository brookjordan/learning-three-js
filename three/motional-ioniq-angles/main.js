...
import { VSMShadowMap, BackSide, DoubleSide } from '../modules/three/src/constants.js';
...
let canvas;
let carContainer = document.querySelector('.car');
if (WEBGL === null || WEBGL === void 0 ? void 0 : WEBGL.isWebGL2Available()) {
  canvas = document.createElement('canvas');
  carContainer === null || carContainer === void 0 ? void 0 : carContainer.append(canvas);
  ...
  carContainer === null || carContainer === void 0 ? void 0 : carContainer.appendChild(warning);
...
scene.add(camera, floor, mainLight, ambientLight);
...
function step() {
  cameraControls === null || cameraControls === void 0 ? void 0 : cameraControls.update();
  ...
  if (!cameraControls) {
    if (ioniqModel) {
      if (shot.car.rotationSpeed === 0) {
        ...
        ioniqModel === null || ioniqModel === void 0 ? void 0 : ioniqModel.rotateZ(shot.car.rotationSpeed);
      }
    }
    camera.position.x =
      (camera.position.x * (sceneParams.motionDamping - 1) + shot.camera.position.x) / sceneParams.motionDamping;
    camera.position.y =
      (camera.position.y * (sceneParams.motionDamping - 1) + shot.camera.position.y) / sceneParams.motionDamping;
    camera.position.z =
      (camera.position.z * (sceneParams.motionDamping - 1) + shot.camera.position.z) / sceneParams.motionDamping;
...
function updateRenderDimensions(dpp = sceneParams.dpp) {
  ...
  sceneParams.size = Math.min(Math.round(carContainer.clientWidth * dpp), Math.round(carContainer.clientHeight * dpp));
...
function updateShot(shotName) {
  shot = shots[shotName];
}
Object.keys(shots).forEach((shotName) => {
  document.querySelector(`.shot--${shotName}`).onclick = (event) => {
