...
let fFreq: number;
let fPers: number;
let aOctFreq: number[]; // frequency per octave
let aOctPers: number[]; // persistance per octave
let fPersMax: number;
...
function noise2D (x: number, y: number, _z?: number) {
        ...
        for (let i=0; i < iOctaves; i++) {
            ...
            switch(arguments.length) {
                case 3 : fResult += fPers * noise3D(fFreq*x, fFreq*y, fFreq*z);
                    break;
                case 2 : fResult += fPers * noise2D(fFreq*x, fFreq*y, undefined);
                    break;
