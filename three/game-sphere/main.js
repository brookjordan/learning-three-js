...
let keysArray = ['w', 'a', 's', 'd', ' '].map((keyName) => [keyName, new Key(keyName)]);
let keys = Object.fromEntries(keysArray);
...
class Entity {
  constructor({ mesh, speed = new Vector3(0, 0, 0), radius = 0.1 }) {
    this.mesh = mesh;
    this.speed = speed;
    this.radius = radius;
    this.age = 0;
...
let character;
{
  ...
  character.mesh.geometry.setAttribute(
    ...
    new BufferAttribute(
      new Float32Array(
        new Array(character.mesh.geometry.getAttribute('position').count)
...
function step() {
  ...
  if (Math.sqrt(character.mesh.position.x ** 2 + character.mesh.position.z ** 2) > 13) {
    character.mesh.position.x = character.speed.x = character.mesh.position.z = character.speed.z = 0;
  }
